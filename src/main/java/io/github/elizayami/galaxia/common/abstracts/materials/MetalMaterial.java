...
public class MetalMaterial
{
	...
	{
		return new MetalMaterial(name, true,
				AbstractBlock.Properties.create(Material.IRON, color).setRequiresTool().hardnessAndResistance(5.0F, 6.0F).sound(SoundType.METAL),
	...
	{
		return new MetalMaterial(name, true,
				AbstractBlock.Properties.create(Material.IRON, color).setRequiresTool().hardnessAndResistance(hardness, resistance).sound(SoundType.METAL),
				new Item.Properties().group(Galaxia.galaxiaGroup), material, armor);
	}

	public static MetalMaterial makeOreless(String name, MaterialColor color, IItemTier material, IArmorMaterial armor)
	{
		return new MetalMaterial(name, false,
				AbstractBlock.Properties.create(Material.IRON, color).setRequiresTool().hardnessAndResistance(5.0F, 6.0F).sound(SoundType.METAL),
				new Item.Properties().group(Galaxia.galaxiaGroup), material, armor);
	}
	...
	public MetalMaterial(String name, boolean hasOre, AbstractBlock.Properties blockSettings, Item.Properties itemSettings, IItemTier material,
			IArmorMaterial armor)
	{
		this.hasOre = hasOre;
		...
		if (name == "cometsteel")
		{
			helmet = ItemInit.registerItem(name + "_helmet", () -> new CometsteelArmor(armor, EquipmentSlotType.HEAD, itemSettings));
			chestplate = ItemInit.registerItem(name + "_chestplate", () -> new CometsteelArmor(armor, EquipmentSlotType.CHEST, itemSettings));
			leggings = ItemInit.registerItem(name + "_leggings", () -> new CometsteelArmor(armor, EquipmentSlotType.LEGS, itemSettings));
			boots = ItemInit.registerItem(name + "_boots", () -> new CometsteelArmor(armor, EquipmentSlotType.FEET, itemSettings));
		}
		else if (name == "silver")
		{
			helmet = ItemInit.registerItem(name + "_helmet", () -> new SilverArmor(armor, EquipmentSlotType.HEAD, itemSettings));
			chestplate = ItemInit.registerItem(name + "_chestplate", () -> new SilverArmor(armor, EquipmentSlotType.CHEST, itemSettings));
			leggings = ItemInit.registerItem(name + "_leggings", () -> new SilverArmor(armor, EquipmentSlotType.LEGS, itemSettings));
			boots = ItemInit.registerItem(name + "_boots", () -> new SilverArmor(armor, EquipmentSlotType.FEET, itemSettings));
		}
		else
		{
