...
public class StoneMaterial 
{
	...
	public StoneMaterial(String name, MaterialColor color) 
	{
		...
		button = BlockInit.registerBlockWithDefaultItem(name + "_button", 
				() -> new StoneButtonBlock(material));
		pressure_plate = BlockInit.registerBlockWithDefaultItem(name + "_pressure_plate", 
				() -> new PressurePlateBlock(Sensitivity.MOBS, material));
		bricks = BlockInit.registerBlockWithDefaultItem(name + "_bricks", 
				() -> new Block(material));
		brick_stairs = BlockInit.registerBlockWithDefaultItem(name + "_bricks_stairs", 
				() -> new StairsBlock(() -> bricks.get().getDefaultState(), material));
		brick_slab = BlockInit.registerBlockWithDefaultItem(name + "_bricks_slab", 
				() -> new SlabBlock(material));
		brick_wall = BlockInit.registerBlockWithDefaultItem(name + "_bricks_wall", 
				() -> new WallBlock(material));
		MATERIALS.add(this);
	}
