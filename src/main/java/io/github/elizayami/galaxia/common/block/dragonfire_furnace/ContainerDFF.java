...
public class ContainerDFF extends Container
{
	...
	public static ContainerDFF createContainerServerSide(int windowID, PlayerInventory playerInventory,
	...
	public static final int FUEL_SLOTS_COUNT = TileEntityDragonfireFurnace.FUEL_SLOTS_COUNT;
	public static final int INPUT_SLOTS_COUNT = TileEntityDragonfireFurnace.INPUT_SLOTS_COUNT;
	public static final int OUTPUT_SLOTS_COUNT = TileEntityDragonfireFurnace.OUTPUT_SLOTS_COUNT;
	...
	public ContainerDFF(int windowID, PlayerInventory invPlayer, FurnaceZoneContents inputZoneContents,
			...
			FurnaceStateData furnaceStateData)
	{
		...
		trackIntArray(furnaceStateData); // tell vanilla to keep the furnaceStateData synchronised between client and
											// server Containers
	...
	// returns ItemStack.EMPTY if the source slot is empty, or if none of the source
	// slot item could be moved.
	...
	public ItemStack transferStackInSlot(PlayerEntity player, int sourceSlotIndex)
	{
		...
		switch (sourceZone)
		{
		case OUTPUT_ZONE: // taking out of the output zone - try the hotbar first, then main inventory.
							// fill from the end.
			...
			if (successfulTransfer)
			{ // removing from output means we have just crafted an item -> need to inform
				sourceSlot.onSlotChange(sourceItemStack, sourceStackBeforeMerge);
			}
			break;

		case INPUT_ZONE:
		case FUEL_ZONE: // taking out of input zone or fuel zone - try player main inv first, then
						// hotbar. fill from the start
		...
		case PLAYER_MAIN_INVENTORY: // taking out of inventory - find the appropriate furnace zone
			if (!TileEntityDragonfireFurnace.getSmeltingResultForItem(world, sourceItemStack).isEmpty())
			{ // smeltable
				// -> add to
				// input
				successfulTransfer = mergeInto(SlotZone.INPUT_ZONE, sourceItemStack, false);
			}
			if (!successfulTransfer && TileEntityDragonfireFurnace.getItemBurnTime(world, sourceItemStack) > 0)
			{ // burnable
				// ->
				// add
				// to
			...
			if (!successfulTransfer)
			{ // didn't fit into furnace; try player main inventory or hotbar
		...
		// If source stack is empty (the entire stack was moved) set slot contents to
		// empty
	 ...
	 * @param fillFromEnd     if true: try to merge from the end of the zone instead
	 *                        of from the start
	...
	public class SlotFuel extends Slot
	{
		...
		{
			return TileEntityDragonfireFurnace.isItemValidForFuelSlot(stack);
		}
	}
	...
	public class SlotSmeltableInput extends Slot
	{
		...
		public boolean isItemValid(ItemStack stack)
		{
			return TileEntityDragonfireFurnace.isItemValidForInputSlot(stack);
		}
	}
	...
	public class SlotOutput extends Slot
	{
		...
		public boolean isItemValid(ItemStack stack)
		{
			return TileEntityDragonfireFurnace.isItemValidForOutputSlot(stack);
		}
	}
