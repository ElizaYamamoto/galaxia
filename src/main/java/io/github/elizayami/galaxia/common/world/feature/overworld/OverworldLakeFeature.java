...
import io.github.elizayami.galaxia.common.world.OpenSimplexNoiseEnd;

import io.github.elizayami.galaxia.util.BlockHelper;
import io.github.elizayami.galaxia.util.FeatureHelper;
import io.github.elizayami.galaxia.util.ModMathHelper;
...
{
	private static final BlockState END_STONE = Blocks.STONE.getDefaultState();
	...
	@Override
	public boolean generate(ISeedReader world, ChunkGenerator chunkGenerator_, Random random, BlockPos blockPos,
					...
					for (int y = blockPos.getY(); y <= blockPos.getY() + 20; y++)
					{
						...
						if (r > 0)
						{
							...
							if (x2 + z2 <= r)
							{
								...
								if (state.equals(Tags.Blocks.STONE))
								{
									BlockHelper.setWithoutUpdate(world, POS, Blocks.AIR.getDefaultState());
								}
								pos = POS.down();
								if (world.getBlockState(pos).equals(Tags.Blocks.STONE))
								{
									...
									if (y > waterLevel + 1)
										...
										BlockHelper.setWithoutUpdate(world, pos,
												random.nextBoolean() ? state : Blocks.GRAVEL.getDefaultState());
									else
										BlockHelper.setWithoutUpdate(world, pos, Blocks.GRAVEL.getDefaultState());
								}
							}
						}
						else
				...
				if (!mask[mx][mz])
				{
					for (int y = blockPos.getY() - bott; y < blockPos.getY(); y++)
					{
						...
						if (y2 + x2 + z2 <= r)
						{
							...
							pos = POS.down();
							if (world.getBlockState(pos).getBlock().equals(Tags.Blocks.STONE))
							{
								BlockHelper.setWithoutUpdate(world, pos, Blocks.GRAVEL.getDefaultState());
							}
							pos = POS.up();
							...
							{
								BlockHelper.setWithoutUpdate(world, pos,
										pos.getY() < waterLevel ? Blocks.WATER : Blocks.AIR);
								pos = pos.up();
							}
						}
						// Make border
	...
	{
		return state.getMaterial().isReplaceable() || state.equals(Tags.Blocks.STONE) || state.equals(BlockTags.FLOWERS)
				|| state.equals(Tags.Blocks.ORES) // Handles floating ores
				|| state.equals(Blocks.PODZOL) // Handles other blocks that could be left floating
				|| state.equals(Blocks.COARSE_DIRT) // Handles other blocks that could be left floating
				|| state.equals(Blocks.DIRT) // Handles other blocks that could be left floating
				|| state.equals(Blocks.GRAVEL) || state.equals(Blocks.GRASS) || state.equals(Blocks.GRASS_BLOCK)
				|| state.getMaterial().equals(Material.TALL_PLANTS) || state.getMaterial().equals(Material.OCEAN_PLANT)
