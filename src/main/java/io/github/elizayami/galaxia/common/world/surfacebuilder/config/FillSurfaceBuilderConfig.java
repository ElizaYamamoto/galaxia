package io.github.elizayami.galaxia.common.world.surfacebuilder.config;
...
public class FillSurfaceBuilderConfig implements ISurfaceBuilderConfig
{
	...
	FillSurfaceBuilderConfig(BlockStateProvider topMaterial, BlockStateProvider underMaterial,
			BlockStateProvider fillMaterial, int fillDownToY, int fillUpToY, List<BlockState> whitelist)
	{
		...
		this.replaceList = whitelist.stream().map(AbstractBlock.AbstractBlockState::getBlock)
	...
	public static class Builder
	{
		...
		{
			return new FillSurfaceBuilderConfig(topMaterial, underMaterial, fillMaterial, fillDownToY, fillUpToY,
					this.replaceList.stream().map(Block::getDefaultState).collect(Collectors.toList()));
		}
	}
}
