...
public class PointedSBConfig implements ISurfaceBuilderConfig
{
	...
	public static final Codec<PointedSBConfig> CODEC = RecordCodecBuilder.create((codecRecorder) ->
	{
		...
		}), BlockStateProvider.CODEC.fieldOf("spike_provider")
				.orElse(new SimpleBlockStateProvider(Blocks.PODZOL.getDefaultState())).forGetter((config) ->
				{
					return config.spikeProvider;
				}), BlockStateProvider.CODEC.fieldOf("spike_top_provider")
						.orElse(new SimpleBlockStateProvider(Blocks.PODZOL.getDefaultState())).forGetter((config) ->
						{
	...
	public PointedSBConfig(BlockState topMaterial, BlockState underMaterial, BlockStateProvider spikeProvider,
			BlockStateProvider spikeTopBlockProvider, int spikeHeight)
	{
