...
public class BiomeDataListHolderSerializer
		implements JsonSerializer<BiomeDataListHolder>, JsonDeserializer<BiomeDataListHolder>
{
		...
		for (BiomeData biomeData : Src.getBiomeData())
		{
			...
			if (biomeData.getBiomeWeightedList() != null)
			{
				for (WeightedList.Entry<Biome> biomeEntry : biomeData.getBiomeWeightedList().field_220658_a)
				{
					...
					{
						Galaxia.LOGGER.error("One or more \"hills\" \"name\" value was null/incorrect.");
					}
				}
			}
	...
	@Override
	public BiomeDataListHolder deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
			throws JsonParseException
	{
		...
		for (Map.Entry<String, JsonElement> elementEntry : entrySet)
		{
			...
			{
				Galaxia.LOGGER.error(elementEntry.getKey() + "'s \"climate\" value is incorrect you put: \"" + climate
						+ "\". Defaulting climate to warm...");
			...
			if (types.size() == 0)
			{
				...
				Galaxia.LOGGER.warn("No dictionary entries were read...defaulting to: \"OVERWORLD\"");
			}
				...
				if (hillBiomeName != null && hillWeight != null)
				{
					...
					if (hillResourceLocation != null)
					{
						if (WorldGenRegistries.BIOME.keySet().contains(hillResourceLocation))
							weightedList.func_226313_a_(Objects.requireNonNull(WorldGenRegistries.BIOME.getOrDefault(hillResourceLocation)),
									hillWeight);
						else
							Galaxia.LOGGER.error("Could not find: \"" + hillResourceLocation.toString()
			...
			{
				if (biomeKey.getNamespace().equals(Galaxia.MOD_ID))
					biomeData.add(new BiomeData(WorldGenRegistries.BIOME.getOrDefault(biomeKey), weight, biomeType, typesArray,
							weightedList, WorldGenRegistries.BIOME.getOrDefault(new ResourceLocation(edge)),
							WorldGenRegistries.BIOME.getOrDefault(new ResourceLocation(beach)),
							WorldGenRegistries.BIOME.getOrDefault(new ResourceLocation(river))));
				else
					Galaxia.LOGGER.error("Biome key: \"" + biomeName
							+ "\" is illegal. The mod id for the biome key MUST be \"byg\". Skipping entry...");
			}
			else
				Galaxia.LOGGER
						.error("The biome key: \"" + biomeName + "\" was not found in the registry, skipping entry...");
		}
