...
public class SubBiomeDataListHolderSerializer
		implements JsonSerializer<SubBiomeDataListHolder>, JsonDeserializer<SubBiomeDataListHolder>
{
	...
	@Override
	public SubBiomeDataListHolder deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
			throws JsonParseException
	{
		Galaxia.LOGGER.info("Reading Sub-Biomes json");
		...
		for (Map.Entry<String, JsonElement> elementEntry : entrySet)
		{
			...
			if (WorldGenRegistries.BIOME.keySet().contains(biomeKey))
			{
				if (biomeKey.getNamespace().equals(Galaxia.MOD_ID))
					biomeData.add(new SubBiomeData(WorldGenRegistries.BIOME.getOrDefault(biomeKey), typesArray,
							WorldGenRegistries.BIOME.getOrDefault(new ResourceLocation(edge)),
