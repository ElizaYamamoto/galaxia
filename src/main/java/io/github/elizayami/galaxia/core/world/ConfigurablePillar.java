...
public class ConfigurablePillar extends Feature<SimpleBlockProviderConfig>
{
	...
	// A copy paste of Minecraft's Basalt Pillar Feature where the blocks can be
	// chosen in a config rather than hardcoded.
			...
			SimpleBlockProviderConfig config)
	{
		if (world.isAirBlock(pos) && !world.isAirBlock(pos.up()))
		{
			...
			while (world.isAirBlock(mutable))
			{
				...
				flag = flag
						&& this.randChanceBlock2(world, rand, mutable2.setAndMove(mutable, Direction.NORTH), config);
				flag1 = flag1
						&& this.randChanceBlock2(world, rand, mutable2.setAndMove(mutable, Direction.SOUTH), config);
				flag2 = flag2
						&& this.randChanceBlock2(world, rand, mutable2.setAndMove(mutable, Direction.WEST), config);
				flag3 = flag3
						&& this.randChanceBlock2(world, rand, mutable2.setAndMove(mutable, Direction.EAST), config);
