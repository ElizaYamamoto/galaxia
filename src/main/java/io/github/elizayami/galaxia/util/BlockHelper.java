package io.github.elizayami.galaxia.util;
		...
		for (int x = start.getX(); x <= end.getX(); x++) 
		{
			...
			for (int z = start.getZ(); z <= end.getZ(); z++) 
			{
				...
				for (int y = start.getY(); y <= end.getY(); y++) 
				{
					...
					{
						// Chorus
						if (state.equals(Blocks.CHORUS_PLANT)) 
						{
							...
							for (int i = 0; i < 64 && !ends.isEmpty(); i++) 
							{
								ends.forEach((pos) -> {
									...
									for (Direction dir : HORIZONTAL_DIRECTIONS) 
									{
										...
										if ((st.equals(Blocks.CHORUS_PLANT) || st.equals(Blocks.CHORUS_FLOWER)) && !st.isValidPosition(world, p)) 
										{
											add.add(p);
										}
									}
